//
//  FritzCore.h
//  FritzCore
//
//  Created by Christopher Kelly on 6/8/18.
//  Copyright © 2018 Fritz Labs Incorporated. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for FritzCore.
FOUNDATION_EXPORT double FritzCoreVersionNumber;

//! Project version string for FritzCore.
FOUNDATION_EXPORT const unsigned char FritzCoreVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <FritzCore/PublicHeader.h>


